...
//
// WHY THIS EXISTS: every deployed handler in this service except
// api/cron/evaluate.js declares `runtime: 'edge'`. Vercel's Edge Runtime is a
// limited V8 isolate with no Node built-ins, and Pino depends on them
// (sonic-boom, worker threads, process.stdout) — so `import pino` throws there.
//
// This emits the SAME newline-delimited JSON shape Pino does, so:
//   - `vercel logs ... | npx pino-pretty` renders it correctly
//   - log processors that understand Pino understand this
//   - a handler moved to `runtime: 'nodejs'` can swap this import for
//     ./logger.node.js with zero call-site changes
//
// Node contexts (scripts/dev-server.mjs, api/cron/evaluate.js) should import
// ./logger.node.js instead — that's real Pino, with pino-pretty locally.
// ---------------------------------------------------------------------------
